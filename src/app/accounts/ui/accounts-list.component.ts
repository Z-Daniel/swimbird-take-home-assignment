    ...
    <ul class="flex flex-col gap-3 md:hidden" role="list">
      @for (account of accounts(); track account.id) {
        <li>
          <a
            [routerLink]="['/accounts', account.id]"
            class="block rounded-lg border border-(--color-border) bg-(--color-surface) p-4 no-underline transition-colors hover:bg-(--color-border) focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          >
            <div class="flex items-start justify-between gap-2">
              <div>
                <p class="font-medium text-(--color-text)">{{ account.name }}</p>
                <p class="text-xs text-(--color-text-muted)">{{ account.type }} · {{ account.currency }}</p>
              </div>
              <span class="rounded-full px-2 py-0.5 text-xs font-medium" [class]="account.status | accountStatus">
                {{ account.status }}
              </span>
            ...
            <div class="mt-3 flex items-end justify-between">
              <p class="text-lg font-semibold text-(--color-text)">
                {{ account.balance | number:'1.0-0' }} {{ account.currency }}
              </p>
